...
- [ ] Compare direct NNRP and OpenAI HTTP/SSE on the same model, engine, GPU, prompt, output limit,
  warmup, and concurrency.
...
- [ ] Update the English and Chinese `nnrp-doc` vLLM design pages to the Preview4 runtime and vLLM
  `0.18.0..<0.27` support band.
- [ ] Verify every documented adapter symbol against the installed package.
- [ ] Record lower-bound and current-line compatibility evidence and every unsupported capability.
