...
- [ ] Add a true in-process benchmark path for `OpenAI HTTP SSE` versus `NNRP submit/result-push`.
- [ ] Measure 4K, 8K, 16K, and 20K input prompts with controlled concurrency 1, 2, and 4.
- [ ] Report TTFT, TPOT, RTT, output-token throughput, cancellation latency, and per-request error rate for both paths.
